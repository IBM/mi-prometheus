...
class DistractionForget(AlgorithmicSeqToSeqProblem):
    ...
    def __getitem__(self, index):
        ...
        # create the target
        target = np.concatenate(y + x, axis=0)
...
if __name__ == "__main__":
    ...
    distractionforget = DistractionForget(params)

    # get a sample
    sample = distractionforget[0]
    print(repr(sample))
    ...
    problem = DataLoader(dataset=distractionforget, batch_size=batch_size, collate_fn=distractionforget.collate_fn,
    ...
    print('time taken to exhaust a dataset of size {}, with a batch size of {}: {}s'
          .format(distractionforget.__len__(), batch_size, time.time() - s))

    # Display single sample (0) from batch.
    batch = next(iter(problem))
    distractionforget.show_sample(batch, 0)
