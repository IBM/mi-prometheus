...
if __name__ == "__main__":
    ...
    distriactionignore = DistractionIgnore(params)

    # get a sample
    sample = distriactionignore[0]
    print(repr(sample))
    ...
    problem = DataLoader(dataset=distriactionignore, batch_size=batch_size, collate_fn=distriactionignore.collate_fn,
    ...
    print('time taken to exhaust a dataset of size {}, with a batch size of {}: {}s'
          .format(distriactionignore.__len__(), batch_size, time.time() - s))

    # Display single sample (0) from batch.
    batch = next(iter(problem))
    distriactionignore.show_sample(batch, 0)
