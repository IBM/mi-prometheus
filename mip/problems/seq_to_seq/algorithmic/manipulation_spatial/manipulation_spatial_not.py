...
"""manipulation_spatial_not.py: Spatial NOT manipulation problem"""
...
if __name__ == "__main__":
    ...
    manipspatialnot = ManipulationSpatialNot(params)

    # get a sample
    sample = manipspatialnot[0]
    print(repr(sample))
    ...
    problem = DataLoader(dataset=manipspatialnot, batch_size=batch_size, collate_fn=manipspatialnot.collate_fn,
    ...
    print('time taken to exhaust a dataset of size {}, with a batch size of {}: {}s'
          .format(manipspatialnot.__len__(), batch_size, time.time() - s))

    # Display single sample (0) from batch.
    batch = next(iter(problem))
    manipspatialnot.show_sample(batch, 0)
