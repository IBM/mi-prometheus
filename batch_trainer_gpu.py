...
def main():
    ...
    if not os.path.isfile(FLAGS.config):
        print(
            'Error: Batch configuration file {} does not exist'.format(
                FLAGS.config))
    ...
    try:
        ...
        print(
            "Error: Coudn't properly parse the {} batch configuration file".format(
                FLAGS.config))
        exit(-1)
    ...
    if 'batch_overwrite' not in batch_dict:
        ...
        # Create temporary file with settings that will be overwritten for all
        # tasks.
        batch_overwrite_file = NamedTemporaryFile(mode='w')
        yaml.dump(batch_dict['batch_overwrite'],
                  batch_overwrite_file, default_flow_style=False)
    ...
    # Create a configuration specific to this batch trainer: set seeds to
    # random and cuda to false.
    gpu_batch_trainer_default_params = {"training": {
        "seed_numpy": -1, "seed_torch": -1, "cuda": True}}
    # Create temporary file
    gpu_batch_trainer_default_params_file = NamedTemporaryFile(mode='w')
    yaml.dump(gpu_batch_trainer_default_params,
    ...
    for task in batch_dict['batch_tasks']:
        try:
            ...
            current_configs = gpu_batch_trainer_default_params_file.name + \
                ',' + task['default_configs']
    ...
    with ThreadPool(processes=max_concurrent_runs) as pool:
        # This contains a list of `AsyncResult` objects. To check if completed
        # and get result.
        thread_results = []
        ...
        for task in experiments_list:
            ...
            # Check every 3 seconds if there is a (supposedly) free GPU to
            # start a task on
            sleep(3)
            while [r.ready() for r in thread_results].count(
