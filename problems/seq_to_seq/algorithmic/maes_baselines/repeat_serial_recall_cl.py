...
"""
repeat_serial_recall_cl.py: Contains definition of serial memorization and repeat recall problem with\
 control markers and command lines
...
class RepeatSerialRecallCommandLines(AlgorithmicSeqToSeqProblem):
    ...
    def __init__(self, params):
        ...
        self.default_values = {'control_bits': self.control_bits,
                               ...
                               'min_recall_number': self.min_recall_number,
                               'max_recall_number': self.max_recall_number
                               }
    ...
    def __getitem__(self, index):
        ...
        inputs = np.zeros([(recall_number + 1) * (seq_length + 1),
        ...
        for r in range(recall_number):
            ...
            inputs[(r + 1) * (seq_length + 1),
            0:self.control_bits] = np.tile(marker_start_aux,
                                           (1))
            inputs[(r + 1) * (seq_length + 1) + 1:(r + 2) * (seq_length + 1),
        ...
        # bits!)
        targets = np.zeros([(recall_number + 1) * (seq_length + 1),
                            self.data_bits],
                           dtype=np.float32)
        ...
        mask = torch.zeros([(recall_number + 1) *
                            (seq_length + 1)]).type(self.app_state.ByteTensor)
        for r in range(recall_number):
        ...
        data_dict['mask'] = mask
        data_dict['num_subsequences'] = recall_number
    ...
    def collate_fn(self, batch):
        ...
        mask = torch.zeros([batch_size, (recall_number + 1) *
                            (seq_length + 1)]).type(self.app_state.ByteTensor)
        for r in range(recall_number):
            mask[:, (r + 1) * (seq_length + 1) +
                    1:(r + 2) * (seq_length + 1)] = 1
...
if __name__ == "__main__":
    ...
    problem = DataLoader(dataset=repeatserialrecallcl, batch_size=batch_size, collate_fn=repeatserialrecallcl.collate_fn,
    ...
    print('time taken to exhaust a dataset of size {}, with a batch size of {}: {}s'
          .format(len(repeatserialrecallcl), batch_size, time.time() - s))

    # Display single sample (0) from batch.
    batch = next(iter(problem))
    repeatserialrecallcl.show_sample(batch, 0)
