...
"""
dual_serial_reverse_recall_cl.py: Contains definition of dual serial-reverse recall problem with control\
 markers and command lines
...
class DualSerialReverseRecallCommandLines(AlgorithmicSeqToSeqProblem):
    ...
    2. Additionally, there is a command line (4th command bit) indicating whether given item is to be stored\
     in memory (0) or recalled (1).
    ...
    def __init__(self, params):
        ...
        self.name = 'DualSerialReverseRecallCommandLines'
    ...
    def __getitem__(self, index):
        ...
        # 1. Generate inputs.
        # Generate input:  [3*SEQ_LENGTH+3, CONTROL_BITS+DATA_BITS]
        inputs = np.zeros([3 * seq_length + 3,
        ...
        inputs[2 * seq_length + 2, 0:self.control_bits] = np.tile(
            marker_start_aux_reverse, (1))
        inputs[2 * seq_length + 3:3 * seq_length + 3,
        0:self.control_bits] = np.tile(ctrl_aux, (seq_length, 1))

        # 2. Generate targets.
        # Generate target:  [3*SEQ_LENGTH+3, DATA_BITS] (only data
        # bits!)
        targets = np.zeros([3 * seq_length + 3,
        ...
        # 3. Generate mask.
        # Generate target mask: [3*SEQ_LENGTH+3]
        mask = torch.zeros([3 * seq_length + 3]
    ...
    def collate_fn(self, batch):
        ...
        # Set start aux serial recall control marker.
        inputs[:, seq_length + 1, 0:self.control_bits] = np.tile(
            marker_start_aux_serial, (batch_size, 1))
        inputs[:,
        ...
        # Set start aux serial reverse control marker.
        inputs[:, 2 * seq_length + 2, 0:self.control_bits] = np.tile(
            marker_start_aux_reverse, (batch_size, 1))
        inputs[:, 2 * seq_length + 3:3 * seq_length + 3,
        0:self.control_bits] = np.tile(ctrl_aux, (batch_size, seq_length, 1))

        # 2. Generate targets.
        # Generate target:  [BATCH_SIZE, 3*SEQ_LENGTH+3, DATA_BITS] (only data
        # bits!)
        targets = np.zeros([batch_size, 3 * seq_length + 3,
                            self.data_bits], dtype=np.float32)
        # Set bit sequence for serial recall.
        targets[:, seq_length + 2:2 * seq_length + 2, :] = bit_seq
        # Set bit sequence for reverse recall.
        ...
        mask = torch.zeros([batch_size, 3 * seq_length + 3]
                           ).type(self.app_state.ByteTensor)
        mask[:, seq_length + 2:2 * seq_length + 2] = 1
        mask[:, 2 * seq_length + 3:] = 1
...
if __name__ == "__main__":
    ...
    dualserialreverserecallcl = DualSerialReverseRecallCommandLines(params)

    # get a sample
    sample = dualserialreverserecallcl[0]
    print(repr(sample))
    ...
    problem = DataLoader(dataset=dualserialreverserecallcl, batch_size=batch_size, collate_fn=dualserialreverserecallcl.collate_fn,
    ...
    print('time taken to exhaust a dataset of size {}, with a batch size of {}: {}s'
          .format(len(dualserialreverserecallcl), batch_size, time.time() - s))

    # Display single sample (0) from batch.
    batch = next(iter(problem))
    dualserialreverserecallcl.show_sample(batch, 0)
