...
class ManipulationSpatialRotation(AlgorithmicSeqToSeqProblem):
    ...
    1.  -1 < num_bits < 1: relative mode, where num_bits
    represents the % of data bits by which every should be shifted

    2. otherwise: absolute number of bits by which the sequence will be
    shifted.
    ...
    def __init__(self, params):
        ...
        self.name = 'ManipulationSpatialRotation'
    ...
    def __getitem__(self, index):
        ...
        targets = np.zeros([2 * seq_length + 2,
        ...
        bit_seq = np.concatenate(
            (bit_seq[:, num_bits:], bit_seq[:, :num_bits]), axis=1)
...
if __name__ == "__main__":
    ...
    manipspatialrot = ManipulationSpatialRotation(params)

    # get a sample
    sample = manipspatialrot[0]
    print(repr(sample))
    ...
    problem = DataLoader(dataset=manipspatialrot, batch_size=batch_size, collate_fn=manipspatialrot.collate_fn,
    ...
    print('time taken to exhaust a dataset of size {}, with a batch size of {}: {}s'
          .format(manipspatialrot.__len__(), batch_size, time.time() - s))

    # Display single sample (0) from batch.
    batch = next(iter(problem))
    manipspatialrot.show_sample(batch, 0)
