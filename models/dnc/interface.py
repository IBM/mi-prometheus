...
class Interface:
    ...
    def update_weight(self, prev_attention, memory, strength, gate, key, shift, sharp):
    ...
    def update_read_weight(self, link, memory, prev_read_weights, read_mode, key, strength):
        ...
        # Calculate the weight to go forward and backwards along the links matrix
        ...
        # Gate between the content similarity, going forwards along the link matrix and going backwards
    ...
    def update_read(self, update_data, prev_interface_tuple, mem): 
        ...
        # update the attention using either the NTM read mechanism (True) or the DNC (False)
        if self.use_ntm_read:
            ...
            read_attention = self.update_weight(prev_read_attention, memory, strength, gate, key, shift, sharp)
            links=prev_links
        else:
 
            read_mode=update_data['read_mode']
            links = self.temporal_linkage.calc_temporal_links(prev_write_attention, prev_links)
            read_attention = self.update_read_weight(links, memory, prev_read_attention, read_mode, key, strength)
    ...
    def update_write(self, update_data, prev_interface_tuple, mem):
        ...
        # update the attention using either the NTM write mechanism (True) or the DNC (False)
        if self.use_ntm_write:
            # Parameters for shift addressing
            shift = update_data['shifts']
            sharp = update_data['sharpening']

            write_attention = self.update_weight(prev_write_attention, memory, strength, gate, key, shift, sharp)
        else:
            write_gate = update_data['write_gate']
            allocation_gate = gate
            write_attention = self.update_write_weight(usage, memory, allocation_gate, write_gate, key, strength)
    ...
    def update_and_edit(self, update_data, prev_interface_tuple, prev_memory_BxMxA):
        ...
        # Step 3: Update read weights using either the current or previous memory
