...
class MetaSingletonABC(SingletonMetaClass, ABCMeta):
    """
    Metaclass that inherits both SingletonMetaClass, and ABCMeta (collection.Mappings' metaclass).
    """
    pass
...
class ParamRegistry(Mapping, metaclass=MetaSingletonABC):
    ...
    def _update_params(self):
        """
        Computes the params from the default param registry superseded by the custom params registry.
        :return: None
        """
    ...
    def add_default_params(self, default_params: dict):
        """
        Appends default params to the registry. This should not be used by the user, but rather set by the objects
        necessitating default values.
    ...
    def add_custom_params(self, custom_params: dict):
        """
        Appends custom parameters to the registry. This is intended for the user to customize the experiments.
    ...
    def add_custom_params_from_yaml(self, yaml_path: str):
        """
        Helper function. Has the same functionality as `add_custom_params`, but loads from path of yaml file.
