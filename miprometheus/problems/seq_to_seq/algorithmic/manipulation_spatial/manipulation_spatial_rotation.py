...
"""
manipulation_spatial_rotate.py: Spatial rotation (bit-shift) for all items in the sequence
...
class ManipulationSpatialRotation(AlgorithmicSeqToSeqProblem):
    """
        Creates input being a sequence of bit patterns and target being the same
        sequence, but with data_bits "bitshifted" by num_shifted_bits to right.

        Offers two modes of operation, depending on the value of num_shifted_bits
        parameter:

        1.  -1 < num_shifted_bits < 1: relative mode, where num_shifted_bits \
        ...
        .. figure:: ../img/algorithmic/manipulation_spatial/manipulation_spatial_rotation.png
            :scale: 80 %
            :align: center

            Exemplary sequence generated by the Manipulation Spatial Rotation problem (target is shifted by 1 bit, without additional command lines).
    ...
    def __init__(self, params):
        ...
        assert self.data_bits >= 2, "Problem requires at least 2 data bits (currently %r)" % self.data_bits

        # Set number of bits to be shifted.
        params.add_default_params({'num_bits_shifted': 0.5})
        self.num_bits_shifted = params['num_bits_shifted']
    ...
    def generate_batch(self, batch_size):
        ...
        # Bit shift.
        targets[:, seq_length + 2:, :] = self.bit_shift(bit_seq, self.num_bits_shifted)
