...
class ScratchPadCommandLines(AlgorithmicSeqToSeqProblem):
    ...
    In this task the system is exposed to several sequences, with the goal of recalling only the last one.

    .. figure:: ../img/algorithmic/recall/scrach_pad.png
        :scale: 80 %
    ...
    3. Additionally, the target subsequence might contains may contain (random) command lines (when number of command bits is > 2).

    4. Generator returns a mask, which (by default) is used for masking the unimportant elements of the output \
