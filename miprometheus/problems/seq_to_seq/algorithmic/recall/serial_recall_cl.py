...
class SerialRecallCommandLines(AlgorithmicSeqToSeqProblem):
    ...
    .. figure:: ../img/algorithmic/recall/serial_recall.png
        :scale: 80 %
        :alt: Exemplary sequence generated by the Serial Recall problem (no additional command lines).
        :align: center

        Exemplary sequence generated by the Serial Recall problem (no additional command lines).
    ...
    3. Additionally, the target subsequence might contains may contain (random) command lines.

    4. Generator returns a mask, which (by default) are used for masking the unimportant elements of the output \
