...
class RepeatReverseRecallCommandLines(AlgorithmicSeqToSeqProblem):
    ...
    In this problem system has to memorize one sequence, and then repeat its items in reverse order a number of times.

    .. figure:: ../img/algorithmic/manipulation_temporal/repeat_reverse_recall.png
        :scale: 80 %
        :align: center

        Exemplary sequence generated by the Repeat Reverse Recall problem (without additional command lines).
