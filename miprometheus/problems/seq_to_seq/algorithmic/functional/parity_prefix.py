...
class ParityPrefix(AlgorithmicSeqToSeqProblem):
    """
    ...
    def __init__(self, params):
        ...
        super(ParityPrefix, self).__init__(params)
        self.name = 'Parity'
    ...
    def generate_batch(self, batch_size):
        ...
        # 1. Generate inputs.
        # Generate input:  [BATCH_SIZE, SEQ_LENGTH+1, DATA_BITS]
        inputs = np.random.binomial(1, self.bias,
                                    (batch_size, seq_length+1, self.data_bits))
        # Set last item to 0
        inputs[:, -1, :] = 0
        ...
        # bits!)
        targets = np.zeros([batch_size, seq_length + 1, self.data_bits], dtype=np.float32)
        # Set bit sequence.
        targets[:, 1:, :] = np.cumsum(inputs[:, :-1, :], axis=1) % 2

        # 3. Generate mask.
        # Generate target mask: [BATCH_SIZE, 2*SEQ_LENGTH+2, 1]
        ptmasks = torch.zeros([batch_size, seq_length + 1, 1]).type(torch.ByteTensor)
        ptmasks[:, 1:, 0] = 1
