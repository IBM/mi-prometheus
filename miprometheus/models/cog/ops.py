...
class FeatureAttention(nn.Module):
	"""
	``FeatureAttention`` shifts and scales the feature maps generated by
	the CNN separately as an attention mechanism.

	"""
	def __init__(self,attention_size,attention_input_size,use_mlp=False):
		"""
		Constructor of the ``FeatureAttention``. Instantiates linear layers.
	...
	def forward(self,inputs_to_attend,attn_gen):
		"""
		Forward pass of ``FeatureAttention``.

		:param inputs_to_attend: Feature maps to attend. 
		:type inputs_to_attend: torch.Tensor

		:param attn_gen: Input to the linear layer to generate attention from.
		:type attn_gen: torch.Tensor
...
class SpatialAttention(nn.Module):
	"""
	``SpatialAttention`` scales all feature maps together spatially.

	"""
	def __init__(self,attention_size, attention_input_size):
		"""
		Constructor of the ``SpatialAttention``. Instantiates linear layers.
	...
	def forward(self,inputs_to_attend,attn_gen):
		"""
		Forward pass of ``SpatialAttention``.
